...
<style>
  ...
  *, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  .component-link {{ display: inline; color: var(--ink); text-decoration: underline;
    text-decoration-color: var(--rule); text-underline-offset: 3px; }}
  .component-link:hover {{ text-decoration-color: var(--ink); }}
...
INVENTORY.forEach(item => {{
  ...
  if (item.type === 'section') {{
    ...
    const rawLink = item.link && !item.link.startsWith('http') ? 'https://' + item.link : item.link;
    const compName = rawLink
      ? `<a href="${{rawLink}}" target="_blank" class="component-link">${{item.component}}</a>`
